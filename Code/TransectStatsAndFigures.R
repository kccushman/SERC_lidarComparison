## TransectStatsAndFigures.R
## KC Cushman
## For: "A primer on forest structure measurement with lidar for ecologists"

## This script reads in transect lidar subsets from MLS, ALS, ULS, and TLS;
## calculate summary statistics for point density and plant area density;
## and creates data figures.
...
#### Setup ####
  ...
  # Define file path for MLS, ALS, ULS (drone), and TLS lidar files
...
#### Make terrain model ####
  ...
  # use previously classified NEON ALS data for entire SERC ha 4
...
#  Make raster template for voxelized point count/density
  ...
  # define voxel size in m
  voxelSz <- 5
  
  # create template for raster data
...
#### Make table of summary stats ####
  ...
  transectSummary <- data.frame(instrument = c("ALS",
                                               "ULS (leaf on)",
                                               "ULS (leaf off)",
                                               "MLS",
                                               "TLS"),
...
pdf(file = "Results/Figure4.pdf",
     width = 6.81, height = 2.3, pointsize = 13)
...
#### NOTE: Figures 5 and 6 are in GEDI_analyzeData.R #### 
...
#### Figure S3. Rasterized metric plots ####
